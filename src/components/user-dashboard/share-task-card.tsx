  ...
  useEffect(() => {
    ...
    async function captureFingerprint() {
      try {
        ...
        /* fingerprint capture failed, proceed without it */
      }
    }
    ...
    <div className="flex flex-col gap-2 w-full min-w-[200px]">
      {!isComplete && (
        <span className="text-[10px] font-semibold text-slate-500">
          {threshold - clickCount === 1
            ? "1 more open needed"
            : `${Math.max(threshold - clickCount, 0)} more opens needed`}
        </span>
      )}
      <div className="flex items-center gap-2">
        ...
        <span className="text-[10px] font-bold text-slate-500 shrink-0">
          {isComplete ? "Done" : `${clickCount}/${threshold}`}
        </span>
      </div>
      {!isComplete ? (
        ...
        <span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-full text-center">
          ✓ Complete — {clickCount} opens
        </span>
      )}
    </div>
  );
